document.getElementById('form').addEventListener('submit', function (event) {
  ...
  const acidDemand = document.querySelector('#acidDemand').value;
  ...
  function calcPh() {
    ...
    function getAcidDose(poolVolume, drops) {
      ...
      // Handle edge cases (exact match or out of range)
      if (lowerVolume === higherVolume) {
        return acidDemandTable[lowerVolume][drops - 1]; // Exact match
      }
  
      // Interpolation for in-between pool volumes
      ...
      return interpolatedDose.toFixed(2); // Return formatted dose
    }
  
    const volumeLiters = volume * 1000; // Convert pool volume to liters
    const acidDemandDrops = parseInt(document.getElementById('acidDemand').value, 10) || 0;
    ...
    if (ph < 7.2) {
      ...
      const dilutionWater = acidDose * 10; // Calculate dilution water amount
      updateStep(
        'ph',
        'Adjust PH Levels',
      ...
      updateStep(
        'ph',
        'Check Acid Demand',
        'Please verify your acid demand test results before proceeding.'
      );
    } 
    else {
      updateStep('ph');
